...
At the core of this analysis is the novel impact factor used in comparing batting and bowling impacts. To determine the formula for both, responsiveness to the match output between the total impact of both teams was prioritized in addition to similarity in the distribution of the impact factors each game.
...
To predict T20I outcomes based on player-level performance metrics, several classification models were implemented and evaluated. Even though there are only 11 players, there is overlap between these categories. There are 28 data cells, 8 batsmen and 6 bowlers per team. At least 3 players per team will be in both categories as an all-rounder. As a baseline, a simple heuristic predicted the team with the higher cumulative impact score as the winner:  
...
The modeling results are organized below in \Autoref{tab:model-performance}. Note the composite score of all three models.
\begin{table}[!h]
\centering
\resizebox{\textwidth}{!}{%
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.79\linewidth]{Example_Comparison.png}
    \caption{Comparison between Hardik Pandya \& Shakib al Hasan in Web App}
    \label{fig:comparison}
\end{figure}
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.79\linewidth]{Past_Match_Model_Output.png}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.79\linewidth]{Future_Match_Prediction.png}
